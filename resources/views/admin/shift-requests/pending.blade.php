        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">Pending Shift Requests</h1>
                        <p class="text-gray-600 mt-1">Review and manage employee shift change requests</p>
                    </div>
            ...
            <!-- Pending Shift Requests List -->
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium text-gray-900">
                        Pending Shift Requests ({{ $shiftRequests->total() }})
                    </h2>
                </div>
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">Requested Date</p>
                                        <p class="text-sm text-gray-600">{{ $request->requested_date->format('M j, Y') }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">Time Changes</p>
                                        @if($request->current_start_time || $request->requested_start_time)
                                            <p class="text-sm text-gray-600">
                                                {{ $request->current_start_time ? $request->current_start_time : 'N/A' }} → 
                                                {{ $request->requested_start_time ? $request->requested_start_time : 'N/A' }}
                                            </p>
                                        @else
                                            <p class="text-sm text-gray-600">See details</p>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <p class="text-sm font-medium text-gray-900 mb-1">Swap Partner</p>
                                    <p class="text-sm text-gray-600">{{ $request->swapWithUser->name }} {{ $request->swapWithUser->lastname }}</p>
                                </div>
                                @endif
...
function showApprovalModal(requestId, employeeName, action) {
    ...
    if (action === 'approve') {
        modalTitle.textContent = 'Approve Shift Request';
        modalMessage.textContent = `Are you sure you want to approve ${employeeName}'s shift request?`;
        ...
        modalTitle.textContent = 'Reject Shift Request';
        modalMessage.textContent = `Are you sure you want to reject ${employeeName}'s shift request?`;
...
document.getElementById('confirmButton').addEventListener('click', function() {
    ...
    // Note: This would need to be implemented with proper routes
    alert(`Shift request ${currentAction} functionality needs to be implemented with proper routes and controller methods.`);
    hideApprovalModal();
});
