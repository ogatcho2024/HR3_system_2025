    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-gradient-to-r from-yellow-400 to-yellow-500 overflow-hidden shadow-lg rounded-2xl border-l-8 border-yellow-600 transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                    <div class="p-6 bg-white">
                        <div class="flex items-center justify-center">
                            <!-- Icon -->
                            <div class="flex-shrink-0">
                                <div class="w-15 h-15 bg-yellow-300 rounded-full flex items-center justify-center shadow-inner">
                ...
                <div class="bg-white overflow-hidden shadow-lg rounded-2xl border-l-8 border-red-500 transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                    <div class="p-6">
                        <div class="flex items-center">
                            <div class="flex-shrink-0">
                                <div class="w-15 h-15 bg-red-500 rounded-full flex items-center justify-center">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white overflow-hidden shadow rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('employee-management.leave-requests') }}" class="text-sm text-blue-600 hover:text-blue-500">View all</a>
                        </div>
                        <div class="flow-root">
                                        ...
                                        <div class="flex items-center space-x-4">
                                            <div class="flex-shrink-0">
                                                @if($request->user->photo)
                                                    <img class="h-8 w-8 rounded-full" src="{{ asset('storage/' . $request->user->photo) }}" alt="{{ $request->user->name }}">
                                                @else
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate">{{ $request->user->name }} {{ $request->user->lastname }}</p>
                ...
                <div class="bg-white overflow-hidden shadow rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-medium text-gray-900">Recent Shift Requests</h3>
                            <a href="{{ route('employee-management.shift-requests') }}" class="text-sm text-blue-600 hover:text-blue-500">View all</a>
                        </div>
                        <div class="flow-root">
                            @if($recentShiftRequests->count() > 0)
                                <ul class="-my-5 divide-y divide-gray-200">
                                    ...
                                    <li class="py-4">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate">{{ $request->user->name }} {{ $request->user->lastname }}</p>
                                                <p class="text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $request->request_type)) }}</p>
                                            </div>
                                            <div>
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                                    Pending
                ...
                <div class="bg-white overflow-hidden shadow rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-medium text-gray-900">Profiles Need Setup</h3>
                            <a href="{{ route('employee-management.employees', ['profile_status' => 'incomplete']) }}" class="text-sm text-blue-600 hover:text-blue-500">View all</a>
                        </div>
                        <div class="flow-root">
                            @if($incompleteProfileUsers->count() > 0)
                                <ul class="-my-5 divide-y divide-gray-200">
                                    @foreach($incompleteProfileUsers as $user)
                                    <li class="py-4">
                                        <div class="flex items-center space-x-4">
                                            <div class="flex-shrink-0">
                                                @if($user->photo)
                                                    <img class="h-8 w-8 rounded-full" src="{{ asset('storage/' . $user->photo) }}" alt="{{ $user->name }}">
                                                @else
                                                    <div class="h-8 w-8 rounded-full bg-gray-300 flex items-center justify-center">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate">{{ $user->name }} {{ $user->lastname }}</p>
                                                <p class="text-sm text-gray-500">{{ $user->email }}</p>
                                            </div>
                                            <div>
                                                <a href="{{ route('employee-management.employees.setup', $user) }}" class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-blue-700 bg-blue-100 hover:bg-blue-200">
                                                    Setup
                                                </a>
                                            </div>
                                        </div>
